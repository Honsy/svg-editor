  ...
  async init() {
    ...
    const addMarker = (id, seType) => {
      ...
      if (type != "textmarker") {
        ...
        const mel = addElem(markerTypes[type])
        mel.textContent = seType;
        const bbox = mel.getBBox();
        let copyBbox = bbox;
        copyBbox.x = 0;
        copyBbox.y = 0;
        copyBbox.width += 2
        copyBbox.height += 2
        mel.setAttribute("x", 1)
        mel.setAttribute("y", copyBbox.height - 1 - bbox.height / 4)
        mel.setAttribute("fill", color)
        refY = copyBbox.width / 2 + 1;
        refX = copyBbox.height / 2 + 1;
        viewBox = copyBbox.x + " " + copyBbox.y + " " + copyBbox.width + " " + copyBbox.height;
        markerWidth = copyBbox.width / 10;
        markerHeight = copyBbox.height / 10;

        const svgDom = addElem({
          element: "rect",
          attr: {
            x: copyBbox.x,
            y: copyBbox.y,
        ...
        mel.setAttribute("orient", 0)
        marker.appendChild(svgDom)
    ...
    return {
      name: `markers`,
      mySetMarker(cmd_maker, optionIndex) {
        console.warn('mySetMarker', cmd_maker, optionIndex)
        ...
        let markerId = "marker-" + cmd;
        let i = "\\" + updateKey;
        let element = selectElements[0]
        let marker = getLinked(element, markerId)
        marker && $(marker).remove()
        element.removeAttribute(markerId);
        if (i == "") {
          i = "\\nomarker";
        }
        ...
        addMarker(pos, i)
        svgCanvas.changeSelectedAttribute(markerId, "url(#" + pos + ")")
