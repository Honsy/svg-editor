...
* @returns {string}
*/
export const regexEscape = function (str: string) {
...
export default class ConfigObj {
  defaultPrefs: any
  defaultConfig: {
    canvasName: string; canvas_expansion: number; initFill: {
      color: string // solid red
      opacity: number
    }; initStroke: {
      width: number; color: string // solid black
      opacity: number
    }; text: { stroke_width: number; font_size: number; font_family: string }; initOpacity: number; initTool: string; exportWindowType: string // 'same' (todo: also support 'download')
    wireframe: boolean; showlayers: boolean; no_save_warning: boolean
    imgPath: string; extPath: string
    dimensions: number[]
    gridSnapping: boolean; gridColor: string; baseUnit: string; snappingStep: number; showRulers: boolean
    dynamicOutput: boolean
    preventAllURLConfig: boolean; preventURLContentLoading: boolean
    lockExtensions: boolean // Disallowed in URL setting
    noDefaultExtensions: boolean // noDefaultExtensions can only be meaningfully used in `svgedit-config-iife.js` or in the URL
    showGrid: boolean // Set by ext-grid.js
    ...
    avoidClientSide: boolean // Deprecated in favor of `avoidClientSideDownload`
    avoidClientSideDownload: boolean; avoidClientSideOpen: boolean
  }
  curPrefs: any
  urldata: any
  ...
  loadFromURL () {
    const self = this
    const { search, searchParams } = new URL(location as any)
    if (search) {
      this.urldata = {}
  ...
  */
  loadContentAndPrefs () {
    ...
    // LOAD PREFS
    Object.keys(this.defaultPrefs).forEach((key: any) => {
...
*/
  setConfig (opts: any, cfgCfg: any = {}) {
     ...
     * @returns {void}
     */
    const extendOrAdd = (cfgObj: any, key: any, val: any) => {
    ...
    Object.entries(opts).forEach(([key, val]) => {
      ...
      // Only allow other configObj.curConfig if defined in configObj.defaultConfig
        ...
        } else if (this.defaultConfig[key as keyof typeof this.defaultConfig] && typeof this.defaultConfig[key as keyof typeof this.defaultConfig] === 'object') {
          this.curConfig[key] = Array.isArray(this.defaultConfig[key as keyof typeof this.defaultConfig]) ? [] : {}
